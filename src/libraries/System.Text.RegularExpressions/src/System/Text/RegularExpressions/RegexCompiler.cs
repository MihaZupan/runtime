...
namespace System.Text.RegularExpressions
{
    ...
    internal abstract class RegexCompiler
    {
        ...
        private static readonly FieldInfo s_indexOfAnyAsciiValuesField = typeof(CompiledRegexRunner).GetField("_indexOfAnyAsciiValues", BindingFlags.Instance | BindingFlags.NonPublic)!;
        ...
        private static readonly MethodInfo s_spanIndexOfAnyAscii = typeof(MemoryExtensions).GetMethod("IndexOfAny", new Type[] { typeof(ReadOnlySpan<char>), typeof(IndexOfAnyAsciiValues) })!;
        private static readonly MethodInfo s_spanIndexOfAnyExceptAscii = typeof(MemoryExtensions).GetMethod("IndexOfAnyExcept", new Type[] { typeof(ReadOnlySpan<char>), typeof(IndexOfAnyAsciiValues) })!;
        private static readonly MethodInfo s_spanLastIndexOfAnyAscii = typeof(MemoryExtensions).GetMethod("LastIndexOfAny", new Type[] { typeof(ReadOnlySpan<char>), typeof(IndexOfAnyAsciiValues) })!;
        private static readonly MethodInfo s_spanLastIndexOfAnyExceptAscii = typeof(MemoryExtensions).GetMethod("LastIndexOfAnyExcept", new Type[] { typeof(ReadOnlySpan<char>), typeof(IndexOfAnyAsciiValues) })!;
        ...
        protected readonly List<IndexOfAnyAsciiValues> _indexOfAnyAsciiValues = new();
        ...
        protected void EmitTryFindNextPossibleStartingPosition()
        {
            ...
            void EmitFixedSet_LeftToRight()
            {
                ...
                if (canUseIndexOf)
                {
                    ...
                    {
                        LoadIndexOfAnyAsciiValues(primarySet.IndexOfAnyAscii!.Value.Chars);
                        Callvirt(primarySet.IndexOfAnyAscii.Value.Negated ? s_spanIndexOfAnyExceptAscii : s_spanIndexOfAnyAscii);
                    }
        ...
        protected void EmitTryMatchAtCurrentPosition()
        {
            ...
            void EmitSingleCharLoop(RegexNode node, RegexNode? subsequent = null, bool emitLengthChecksIfRequired = true)
            {
                ...
                if (!rtl && subsequent?.FindStartingLiteral() is RegexNode.StartingLiteralData literal)
                {
                    ...
                    else
                    {
                        ...
                        {
                            LoadIndexOfAnyAsciiValues(literal.AsciiSetChars);
                            Callvirt(literal.Negated ? s_spanLastIndexOfAnyExceptAscii : s_spanLastIndexOfAnyAscii);
                        }
            ...
            void EmitSingleCharLazy(RegexNode node, RegexNode? subsequent = null, bool emitLengthChecksIfRequired = true)
            {
                     ...
                     (literal.Range.LowInclusive <= node.Ch && node.Ch <= literal.Range.HighInclusive)))
                {
                    ...
                    else if (literal.AsciiSetChars is not null) // set of only ASCII characters
                    {
                        ...
                        LoadIndexOfAnyAsciiValues(literal.AsciiSetChars);
                        Callvirt(s_spanIndexOfAnyAscii);
                    }
                    ...
                    subsequent?.FindStartingLiteral() is RegexNode.StartingLiteralData literal2)
                {
                    ...
                    {
                        LoadIndexOfAnyAsciiValues(literal2.AsciiSetChars);
                        Callvirt(literal2.Negated ? s_spanIndexOfAnyExceptAscii : s_spanIndexOfAnyAscii);
                    }
                    else
                    {
            ...
            void EmitSingleCharAtomicLoop(RegexNode node)
            {
                    ...
                    RegexCharClass.TryGetAsciiSetChars(node.Str, out char[]? asciiChars))
                {
                    ...
                    LoadIndexOfAnyAsciiValues(asciiChars);
                    Callvirt(RegexCharClass.IsNegated(node.Str) ? s_spanIndexOfAnyAscii : s_spanIndexOfAnyExceptAscii);
                    Stloc(iterationLocal);
        ...
        /// <summary>
        /// Adds an entry in <see cref="CompiledRegexRunner._indexOfAnyAsciiValues"/> for the given <paramref name="asciiChars"/> and emits a load of that initialized value.
        /// </summary>
        private void LoadIndexOfAnyAsciiValues(char[] asciiChars)
        {
            int index = _indexOfAnyAsciiValues.Count;
            _indexOfAnyAsciiValues.Add(MemoryExtensions.CreateIndexOfAnyAsciiValues(asciiChars));

            // this._indexOfAnyAsciiValues[42]
            Ldthisfld(s_indexOfAnyAsciiValuesField);
            _ilg!.Emit(OpCodes.Ldc_I4_S, index);
            _ilg.Emit(OpCodes.Ldelem);
        }
    }
}
