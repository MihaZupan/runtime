...
namespace System.Text.RegularExpressions.Generator
{
    public partial class RegexGenerator
    {
        ...
        /// <summary>Adds an IndexOfAnyAsciiValues initialization helper to the required helpers collection.</summary>
        private static string IndexOfAnyAsciiValues(char[] asciiChars, Dictionary<string, string[]> requiredHelpers)
        {
            ...
            string helperName = $"Ascii_{BitConverter.ToString(bitmap).Replace("-", string.Empty).TrimStart('0')}";
            ...
            if (!requiredHelpers.ContainsKey(helperName))
            {
                ...
                {
                    $"internal static readonly IndexOfAnyAsciiValues {helperName} =",
                    $"    MemoryExtensions.CreateIndexOfAnyAsciiValues({Literal(new string(asciiChars))});",
                });
            }
            ...
            void EmitFixedSet_LeftToRight()
            {
                ...
                if (canUseIndexOf)
                {
                    ...
                    string indexOf =
                        ...
                        {
                            false => $"{span}.IndexOfAny({IndexOfAnyAsciiValues(primarySet.IndexOfAnyAscii.Value.Chars, requiredHelpers)})",
                            true => $"{span}.IndexOfAnyExcept({IndexOfAnyAsciiValues(primarySet.IndexOfAnyAscii.Value.Chars, requiredHelpers)})",
                        };
        ...
        private static void EmitTryMatchAtCurrentPosition(IndentedTextWriter writer, RegexMethod rm, Dictionary<string, string[]> requiredHelpers)
        {
            ...
            void EmitSingleCharLoop(RegexNode node, RegexNode? subsequent = null, bool emitLengthChecksIfRequired = true)
            {
                ...
                if (!rtl && subsequent?.FindStartingLiteral() is RegexNode.StartingLiteralData literal)
                {
                    ...
                    else
                    {
                        ...
                        literal.AsciiSetChars is not null ? $"{lastIndexOfAnyName}({IndexOfAnyAsciiValues(literal.AsciiSetChars, requiredHelpers)}" :
                ...
                if ((node.Options & RegexOptions.RightToLeft) == 0)
                {
                         ...
                         (literal.Range.LowInclusive <= node.Ch && node.Ch <= literal.Range.HighInclusive)))
                    {
                        ...
                        else if (literal.AsciiSetChars is not null) // set of only ASCII characters
                        {
                            ...
                            writer.WriteLine($"{startingPos} = {sliceSpan}.IndexOfAny({IndexOfAnyAsciiValues(literal.AsciiSetChars, requiredHelpers)});");
                        }
                        ...
                        subsequent?.FindStartingLiteral() is RegexNode.StartingLiteralData literal2)
                    {
                        ...
                        writer.WriteLine(
                            ...
                            literal2.AsciiSetChars is not null ? $"{indexOfAnyName}({IndexOfAnyAsciiValues(literal2.AsciiSetChars, requiredHelpers)});" :
            ...
            void EmitSingleCharAtomicLoop(RegexNode node, bool emitLengthChecksIfRequired = true)
            {
                    ...
                    RegexCharClass.TryGetAsciiSetChars(node.Str, out char[]? asciiChars))
                {
                    ...
                    writer.WriteLine($".{indexOfMethod}({IndexOfAnyAsciiValues(asciiChars, requiredHelpers)});");
                    using (EmitBlock(writer, $"if ({iterationLocal} < 0)"))
                    {
                        writer.WriteLine(sliceStaticPos > 0 ?
                            $"{iterationLocal} = {sliceSpan}.Length - {sliceStaticPos};" :
