...
namespace System.Net.ServerSentEvents
{
    ...
    public sealed class SseParser<T>
    {
        ...
        private ArrayBuffer _lineBuffer; // mutable struct, do not make this readonly
        ...
        private ArrayBuffer _dataBuffer = new(initialSize: 0, usePool: true); // mutable struct, do not make this readonly
        ...
        public IEnumerable<SseItem<T>> Enumerate()
        {
            ...
            _lineBuffer = new ArrayBuffer(DefaultArrayPoolRentSize, usePool: true);
            try
            {
        ...
        private bool TryProcessLine(out SseItem<T>? sseItem)
        {
            ...
            Debug.Assert(searchOffset <= lineBuffer.Length, $"{searchOffset}, {lineBuffer.Length}");
            ...
            if (fieldName.SequenceEqual("data"u8))
            {
                ...
                if (!_dataAppended)
                {
                        ...
                        (remainder[0] is LF || (remainder[0] is CR && remainder.Length > 1)))
                    {
                        ...
                        // Move past the line and the following empty line.
        ...
        {
            if (_lineBuffer.ActiveLength == _lineBuffer.Capacity &&
                _lineBuffer.ActiveLength >= _maxBufferSize)
            {
                throw new InvalidDataException(SR.InvalidDataException_SseExceededMaxLength);
            }

            _lineBuffer.EnsureAvailableSpace(1);
        }
