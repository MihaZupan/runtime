    ...
    internal sealed class HttpConnectionPool : IDisposable
    {
        ...
        private async Task AddHttp2ConnectionAsync(RequestQueueItem<Http2Connection?> queueItem)
        {
            ...
            using (CancellationTokenSource cts = GetConnectTimeoutCancellationTokenSource())
            {
                try
                {
                    (Socket? socket, Stream stream, TransportContext? transportContext) = await ConnectAsync(queueItem.Request, true, cts.Token).ConfigureAwait(false);
                    ...
                    if (IsSecure)
                    {
                        ...
                        else
                        {
                            ...
                            await HandleHttp11Downgrade(queueItem.Request, socket, stream, transportContext, cts.Token).ConfigureAwait(false);
                            return;
                        }
                    }
                    else
