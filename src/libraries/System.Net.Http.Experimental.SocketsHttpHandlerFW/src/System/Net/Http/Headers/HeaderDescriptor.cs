...
namespace System.Net.Http.Headers
{
    ...
    internal readonly struct HeaderDescriptor : IEquatable<HeaderDescriptor>
    {
        ...
        private static readonly Encoding Utf8Throw = new UTF8Encoding(encoderShouldEmitUTF8Identifier: false, throwOnInvalidBytes: true);

        private static unsafe bool TryDecodeUtf8(ReadOnlySpan<byte> input, [NotNullWhen(true)] out string? decoded)
        {
            try
            {
                fixed (byte* inputPtr = input)
                {
                    decoded = Utf8Throw.GetString(inputPtr, input.Length);
                    return true;
                }
            }
            catch
