...
namespace System.Net.Http
{
    internal sealed partial class HttpConnectionPool
    {
        ...
        private async Task InjectNewHttp2ConnectionAsync(RequestQueue<Http2Connection?>.QueueItem queueItem)
        {
            ...
            try
            {
                ...
                if (IsSecure)
                {
                    ...
                    {
                        stream.Dispose();
                        connectionException = new HttpRequestExceptionEx(SR.Format(SR.net_ssl_http2_requires_tls12, sslStream.SslProtocol));
                    }
                    else
                    {
        ...
        {
            stream = await ApplyPlaintextFilterAsync(async: true, stream, HttpVersionEx.Version20, request, cancellationToken).ConfigureAwait(false);
            ...
            catch (Exception e)
            {
                ...
                throw new HttpRequestExceptionEx(SR.net_http_client_execution_error, e);
            }
        ...
        private void ReturnHttp2Connection(Http2Connection connection, bool isNewConnection, HttpConnectionWaiter<Http2Connection?>? initialRequestWaiter = null)
        {
            ...
            if (isNewConnection)
            {
                ...
                HttpRequestException hre = new HttpRequestExceptionEx(SR.net_http_http2_connection_not_established);
