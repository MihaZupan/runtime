    ...
    internal sealed partial class Http2Connection
    {
        ...
        private struct Http2StreamWindowManager
        {
            ...
            private void AdjustWindowDynamic(int bytesConsumed, Http2Stream stream)
            {
                ...
                {
                    TimeSpan dt = TimeSpan.FromSeconds((currentTime - _lastWindowUpdate) / (double)Stopwatch.Frequency);
        ...
        private struct RttEstimator
        {
            ...
            private void RefreshRtt(Http2Connection connection)
            {
                ...
                TimeSpan currentRtt = TimeSpan.FromSeconds((Stopwatch.GetTimestamp() - _pingSentTimestamp) / (double)Stopwatch.Frequency);
