...
namespace System.Net.Http
{
    internal sealed partial class Http2Connection : HttpConnectionBase
    {
        ...
        private static readonly byte[] s_protocolLiteralHeaderBytes = new byte[] { 0x0, 0x9, 0x3a, 0x70, 0x72, 0x6f, 0x74, 0x6f, 0x63, 0x6f, 0x6c };
        private static ReadOnlySpan<byte> ProtocolLiteralHeaderBytes => s_protocolLiteralHeaderBytes;
        ...
        private static ReadOnlySpan<byte> Http2ConnectionPreface => s_http2ConnectionPreface;
            ...
            : base(pool, remoteEndPoint)
        {
            ...
            _nextPingRequestTimestamp = EnvironmentEx.TickCount64 + _keepAlivePingDelay;
        ...
        public async ValueTask SetupAsync(CancellationToken cancellationToken)
        {
            try
            {
                ...
                await _outgoingBuffer.WriteAsync(_stream, cancellationToken).ConfigureAwait(false);
        ...
        private async Task FlushOutgoingBytesAsync()
        {
            ...
            if (_outgoingBuffer.ActiveLength > 0)
            {
                try
                {
                    await _outgoingBuffer.WriteAsync(_stream).ConfigureAwait(false);
                }
                catch (Exception e)
                {
                    Abort(e);
        ...
        private async ValueTask<FrameHeader> ReadFrameAsync(bool initialFrame = false)
        {
            ...
            if (_incomingBuffer.ActiveLength < FrameHeader.Size)
            {
                do
                {
                    ...
                    int bytesRead = await _incomingBuffer.ReadAsync(_stream).ConfigureAwait(false);
        ...
        private abstract class WriteQueueEntry : TaskCompletionSource<bool>
        {
        ...
        {
            if (NetEventSource.Log.IsEnabled()) Trace($"{nameof(values)}={string.Join(Encoding.ASCII.GetString(separator ?? Array.Empty<byte>()), values.ToArray())}");
        ...
        private int WriteHeaderCollection(HttpRequestMessage request, HttpHeaders headers, ref ArrayBuffer headerBuffer)
        {
            ...
            foreach (var header in headers)
            {
                ...
                if (!HeaderDescriptor.TryGet(header.Key, out HeaderDescriptor key))
                {
                    Debug.Fail("Invalid header?");
                    continue;
                }

                ReadOnlySpan<string> headerValues = header.Value as string[] ?? header.Value.ToArray();

                Encoding? valueEncoding = encodingSelector?.Invoke(key.Name, request);

                KnownHeader? knownHeader = key.KnownHeader;
                if (knownHeader != null)
                {
                    ...
                    if (knownHeader != KnownHeaders.Host && knownHeader != KnownHeaders.Connection && knownHeader != KnownHeaders.Upgrade && knownHeader != KnownHeaders.ProxyConnection)
                    {
                        ...
                        byte[]? separator = headerValues.Length > 1 ? key.SeparatorBytes : null;
                ...
                else
                {
                    ...
                    WriteLiteralHeader(key.Name, headerValues, valueEncoding, ref headerBuffer);
                }
            }
        ...
        {
            return method is not ("GET" or "OPTIONS" or "DELETE" or "CONNECT" or "HEAD");
        }
        ...
        private void WriteHeaders(HttpRequestMessage request, ref ArrayBuffer headerBuffer)
        {
            ...
            byte[] methodBytes = request.Method.Method switch
            {
                "GET" => new byte[] { 0x80 | H2StaticTable.MethodGet },
                "POST" => new byte[] { 0x80 | H2StaticTable.MethodPost },
                _ => HPackEncoder.EncodeLiteralHeaderFieldWithoutIndexingToAllocatedArray(H2StaticTable.MethodGet, request.Method.Method)
            };

            WriteBytes(methodBytes, ref headerBuffer);

            WriteIndexedHeader(_pool.IsSecure ? H2StaticTable.SchemeHttps : H2StaticTable.SchemeHttp, ref headerBuffer);

            if (request.Headers.Host is string host)
            {
            ...
            if (request.Content == null)
            {
                ...
                if (MustHaveRequestBody(request.Method.Method))
                {
            ...
            {
                throw new HttpRequestExceptionEx(SR.Format(SR.net_http_request_headers_exceeded_length, maxHeaderListSize));
            }
        }
        ...
        public async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                bool shouldExpectContinue = (request.Content != null && request.Headers.ExpectContinue == true);
                ...
                bool duplex = request.Content != null;
            ...
            {
                throw new HttpRequestExceptionEx(e.HttpRequestError, e.Message, e);
            }
            catch (Exception e) when (e is IOException || e is ObjectDisposedException || e is InvalidOperationException)
            {
                throw new HttpRequestExceptionEx(HttpRequestError.Unknown, SR.net_http_client_execution_error, e);
            }
        }
        ...
        private void VerifyKeepAlive()
        {
            ...
            long now = EnvironmentEx.TickCount64;
        ...
        private static void ThrowRetry(string message, Exception? innerException = null) =>
            throw new HttpRequestExceptionEx((innerException as HttpIOException)?.HttpRequestError ?? HttpRequestError.Unknown, message, innerException, RequestRetryType.RetryOnConnectionFailure);
