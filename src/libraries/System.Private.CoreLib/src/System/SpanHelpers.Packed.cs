            ...
            where TResultMapper : struct, IResultMapper<TResult>
        {
            ...
            if (length < Vector128<short>.Count)
            {
                ...
                if (length >= 4)
                {
                    ...
                    if (TNegator.NegateIfNeeded(TTransform.TransformInput(Unsafe.Add(ref searchSpace, 1)) == value)) TResultMapper.ScalarResult(1);
                    if (TNegator.NegateIfNeeded(TTransform.TransformInput(Unsafe.Add(ref searchSpace, 2)) == value)) TResultMapper.ScalarResult(2);
                    if (TNegator.NegateIfNeeded(TTransform.TransformInput(Unsafe.Add(ref searchSpace, 3)) == value)) TResultMapper.ScalarResult(3);
