...
namespace System.Buffers
{
    internal sealed class Any3CharPackedSearchValues : SearchValues<char>
    {
        ...
        internal override bool ContainsAny(ReadOnlySpan<char> span) =>
            PackedSpanHelpers.ContainsAny(ref MemoryMarshal.GetReference(span), _e0, _e1, _e2, span.Length);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        [CompExactlyDependsOn(typeof(Sse2))]
        internal override bool ContainsAnyExcept(ReadOnlySpan<char> span) =>
            PackedSpanHelpers.ContainsAnyExcept(ref MemoryMarshal.GetReference(span), _e0, _e1, _e2, span.Length);
    }
}
