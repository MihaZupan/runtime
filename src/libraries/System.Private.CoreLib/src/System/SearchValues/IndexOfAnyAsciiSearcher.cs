...
namespace System.Buffers
{
    internal static class IndexOfAnyAsciiSearcher
    {
            ...
            where T : struct, IUnsignedNumber<T>
        {
            ...
            if (!Vector512.IsHardwareAccelerated || !Avx512Vbmi.IsSupported || values.Length > 64 || maxInclusive > 127)
            {
                return false;
            }
...
#pragma warning disable IntrinsicsInSystemPrivateCoreLibAttributeNotSpecificEnough
            ...
            where TResultMapper : struct, IResultMapper<short, TResult>
        {
            ...
            {
                Vector512<byte> values = state.ValuesByLow6Bits;
                ...
                if (searchSpaceLength > 2 * Vector512<short>.Count)
                {
                    ...
                    do
                    {
                        ...
                        Vector512<byte> result = IndexOfAnyLookupUniqueLow6Bits<TNegator, TOptimizations>(source0, source1, values);
            ...
            else if (searchSpaceLength > Vector256<byte>.Count)
            {
                ...
                if (searchSpaceLength > Vector512<byte>.Count)
                {
                    ...
                    do
                    {
                        ...
                        Vector512<byte> result = TNegator.NegateIfNeeded(IndexOfAnyLookupUniqueLow6BitsCore(source, values));
                        if (result != Vector512<byte>.Zero)
                        {
                            return TResultMapper.FirstIndex<TNegator>(ref searchSpace, ref currentSearchSpace, result);
                        }
...
#pragma warning restore IntrinsicsInSystemPrivateCoreLibAttributeNotSpecificEnough
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        [CompExactlyDependsOn(typeof(Avx512Vbmi))]
        private static Vector512<byte> IndexOfAnyLookupUniqueLow6Bits<TNegator, TOptimizations>(Vector512<short> source0, Vector512<short> source1, Vector512<byte> valuesByLow6Bits)
            where TNegator : struct, INegator
            where TOptimizations : struct, IOptimizations
        {
            Vector512<byte> source = TOptimizations.PackSources(source0.AsUInt16(), source1.AsUInt16());

            Vector512<byte> result = IndexOfAnyLookupUniqueLow6BitsCore(source, valuesByLow6Bits);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        [CompExactlyDependsOn(typeof(Avx512Vbmi))]
        private static Vector512<byte> IndexOfAnyLookupUniqueLow6BitsCore(Vector512<byte> source, Vector512<byte> valuesByLow6Bits)
        {
            Vector512<byte> values = Avx512Vbmi.PermuteVar64x8(valuesByLow6Bits, source);
            return Vector512.Equals(source, values);
        }
