...
{
    internal static class PerfectHashCharLookup
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ref readonly TValue GetValueRefOrNullRef<TValue>(char[] hashEntries, uint multiplier, TValue[] valueEntries, char key)
        {
            Debug.Assert(hashEntries.Length == valueEntries.Length);

            ulong offset = FastMod(key, (uint)hashEntries.Length, multiplier);
            Debug.Assert(offset < (ulong)hashEntries.Length);

            if (Unsafe.Add(ref MemoryMarshal.GetArrayDataReference(hashEntries), (nuint)offset) == key)
            {
                return ref Unsafe.Add(ref MemoryMarshal.GetArrayDataReference(valueEntries), (nuint)offset);
            }
