...
{
    internal sealed class IndexOfAnyAsciiStringValuesTeddy256NonBucketizedN3<TStartCaseSensitivity, TCaseSensitivity> : IndexOfAnyStringValuesRabinKarp<TCaseSensitivity>
        ...
        where TCaseSensitivity : struct, ICaseSensitivity
    {
        ...
        public IndexOfAnyAsciiStringValuesTeddy256NonBucketizedN3(ReadOnlySpan<string> values, RabinKarp rabinKarp, HashSet<string> uniqueValues) : base(rabinKarp, uniqueValues)
        {
            _values = new EightPackedReferences<string>(values);

            (_n0Low, _n0High) = GenerateNonBucketizedFingerprint256(values, offset: 0);
            (_n1Low, _n1High) = GenerateNonBucketizedFingerprint256(values, offset: 1);
            (_n2Low, _n2High) = GenerateNonBucketizedFingerprint256(values, offset: 2);
        }
