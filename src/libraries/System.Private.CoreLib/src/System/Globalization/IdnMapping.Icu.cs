...
namespace System.Globalization
{
    ...
    {
        private unsafe string IcuConvertCore(ReadOnlySpan<char> source, string? backingString, bool fromUnicode)
        {
            ...
            fixed (char* pSource = &MemoryMarshal.GetReference(source))
            {
                ...
                fixed (char* pOutput = &MemoryMarshal.GetReference(output))
                {
                    actualLength = fromUnicode
                        ? Interop.Globalization.ToAscii(flags, pSource, source.Length, pOutput, output.Length)
                        : Interop.Globalization.ToUnicode(flags, pSource, source.Length, pOutput, output.Length);
                }
        ...
        private unsafe bool IcuTryConvertCore(ReadOnlySpan<char> source, Span<char> destination, out int charsWritten, bool fromUnicode)
        {
            Debug.Assert(!GlobalizationMode.Invariant);
            Debug.Assert(!GlobalizationMode.UseNls);
            ...
            fixed (char* pDestination = &MemoryMarshal.GetReference(destination))
            {
                charsWritten = fromUnicode
                    ? Interop.Globalization.ToAscii(flags, pSource, source.Length, pDestination, destination.Length)
                    : Interop.Globalization.ToUnicode(flags, pSource, source.Length, pDestination, destination.Length);
