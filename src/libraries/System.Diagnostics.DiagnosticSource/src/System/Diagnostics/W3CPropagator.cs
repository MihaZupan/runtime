...
namespace System.Diagnostics
{
    internal sealed class W3CPropagator : DistributedContextPropagator
    {
        ...
        internal static string? ValidateTraceState(string? traceState)
        {
            ...
            Debug.Assert(processed <= traceState.Length);
            ...
            if (processed > 0)
            {
                ...
                if (processed > 0)
                {
                    return traceState.Substring(0, processed);
                }
            }
        ...
        private static bool TryDecodeBaggageValue(ReadOnlySpan<char> valueSpan, out string value)
        {
            ...
            {
                int percentIndex = valueSpan.IndexOf(Percent);
                if (percentIndex < 0)
                {
                    vsb.Append(valueSpan);
                    break;
                }

                vsb.Append(valueSpan.Slice(0, percentIndex));
                valueSpan = valueSpan.Slice(percentIndex);

                if (!TryDecodeEscapedByte(valueSpan, out byte b0))
                {
                    return false;
                }
                ...
                {
                    if (valueSpan.Length < 6 || !TryDecodeEscapedByte(valueSpan.Slice(3), out byte b1) || (b1 & 0xC0) != 0x80)
                    {
                ...
                {
                    if (valueSpan.Length < 9 ||
                        !TryDecodeEscapedByte(valueSpan.Slice(3), out byte b1) ||
                        !TryDecodeEscapedByte(valueSpan.Slice(6), out byte b2) ||
                ...
                {
                    if (valueSpan.Length < 12 ||
                        !TryDecodeEscapedByte(valueSpan.Slice(3), out byte b1) ||
                        !TryDecodeEscapedByte(valueSpan.Slice(6), out byte b2) ||
                        !TryDecodeEscapedByte(valueSpan.Slice(9), out byte b3) ||
        ...
        private static bool TryDecodeEscapedByte(ReadOnlySpan<char> span, out byte value)
        {
            ...
            if (span.Length < 3 ||
                span[0] != Percent ||
                !TryDecodeHexDigit(span[1], out byte byte1) ||
                !TryDecodeHexDigit(span[2], out byte byte2))
            {
                value = 0;
                return false;
            }
        ...
        {
            value = (byte)HexConverter.FromChar(c);
        ...
        // keychar    = lcalpha / DIGIT / "_" / "-"/ "*" / "/" / "@"
        // lcalpha    = %x61-7A ; a-z
        private static readonly SearchValues<char> s_validTraceStateKeyChars = SearchValues.Create("*-/@_abcdefghijklmnopqrstuvwxyz");

        private static bool IsInvalidTraceStateKey(ReadOnlySpan<char> key) => key.IsEmpty || (key[0] < 'a' || key[0] > 'z') || key.ContainsAnyExcept(s_validTraceStateKeyChars);
        ...
        /// </remarks>
        private static bool EncodeBaggageKey(ReadOnlySpan<char> key, ref ValueStringBuilder vsb)
        {
            key = Trim(key);
        ...
        internal static void EncodeBaggageValue(ReadOnlySpan<char> value, ref ValueStringBuilder vsb)
        {
            ...
            for (int i = 0; i < value.Length; i++)
            {
                ...
                if (s_validBaggageChars.Contains(c))
                {
                    ReadOnlySpan<char> remaining = value.Slice(i);

                    int validChars = remaining.IndexOfAnyExcept(s_validBaggageChars);
                    if (validChars < 0)
                    {
                        validChars = remaining.Length;
                    }

                    vsb.Append(remaining.Slice(0, validChars));
                    i += validChars - 1; // -1 because we will increment i in the loop
                    continue;
                }
        ...
        private static readonly SearchValues<char> s_validBaggageChars = SearchValues.Create("!#$&'()*+-./0123456789:<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[]^_`abcdefghijklmnopqrstuvwxyz{|}~");
        ...
        private static bool IsInvalidTraceParent(string? traceParent)
        {
            ...
            // version = 2HEXDIGLC; this document assumes version 00. Version ff is forbidden
            if (!HexConverter.IsHexLowerChar(traceParent[0]) || !HexConverter.IsHexLowerChar(traceParent[1]) || (traceParent[0] == 'f' && traceParent[1] == 'f'))
            {
                return true;
            }
            ...
            // parent-id = 16HEXDIGLC  ; 8 bytes array identifier. All zeroes forbidden
            if (!ActivityTraceId.IsLowerCaseHexAndNotAllZeros(traceParent.AsSpan(36, 16)))
            {
                return true;
            }
