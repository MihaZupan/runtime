...
namespace System
{
    ...
    public partial class Uri : ISerializable
    {
        ...
        internal static string InternalEscapeString(string rawString) =>
            ...
            UriHelper.EscapeString(rawString, checkExistingEscaped: true, UriHelper.UnreservedReservedExceptQueryFragmentChars);
            ...
            if (host.Length != 0)
            {
                ...
                if (HostType == Flags.BasicHostType)
                {
                    ...
                    if ((result & (Check.EscapedCanonical | Check.BackslashInPath)) != Check.EscapedCanonical)
                    {
                        ...
                        {
                            host = UriHelper.EscapeString(host, checkExistingEscaped: !IsImplicitFile, UriHelper.UnreservedReservedExceptQueryFragmentChars);
                        }
                        else
                        {
                ...
                if ((nonCanonical & (ushort)UriComponents.UserInfo) != 0)
                {
                    switch (formatAs)
                    {
                        case UriFormat.UriEscaped:
                            if (NotAny(Flags.UserEscaped))
                            {
                                UriHelper.EscapeString(slice, ref dest, checkExistingEscaped: true, UriHelper.UnreservedReservedExceptQueryFragmentChars);
                            }
                            else
                            {
                    ...
                    if (formatAs == UriFormat.UriEscaped)
                    {
                        if (NotAny(Flags.UserEscaped))
                        {
                            UriHelper.EscapeString(
                                ...
                                ref dest, checkExistingEscaped: true, UriHelper.UnreservedReservedExceptFragmentChars);
                    ...
                    if (formatAs == UriFormat.UriEscaped)
                    {
                        if (NotAny(Flags.UserEscaped))
                        {
                            UriHelper.EscapeString(
                                ...
                                ref dest, checkExistingEscaped: true, UriHelper.UnreservedReservedChars);
        ...
        private unsafe void GetCanonicalPath(ref ValueStringBuilder dest, UriFormat formatAs)
        {
            ...
            if (formatAs == UriFormat.UriEscaped)
            {
                ...
                else
                {
                    ...
                    if (InFact(Flags.E_PathNotCanonical) && NotAny(Flags.UserEscaped))
                    {
                        ...
                        UriHelper.EscapeString(
                            ...
                            ref dest, checkExistingEscaped: !IsImplicitFile, UriHelper.UnreservedReservedExceptQueryFragmentChars);
                    }
                    else
                    {
                        dest.Append(_string.AsSpan(_info.Offset.Path, _info.Offset.Query - _info.Offset.Path));
                ...
                if (!OperatingSystem.IsWindows() && InFact(Flags.BackslashInPath) && _syntax.NotAny(UriSyntaxFlags.ConvertPathSlashes) && _syntax.InFact(UriSyntaxFlags.FileLikeUri) && !IsImplicitFile)
                {
                    ...
                    UriHelper.EscapeString(copySpan, ref dest, checkExistingEscaped: true, UriHelper.UnreservedReservedChars);
                    start = dest.Length;
            ...
            if (InFact(Flags.ShouldBeCompressed) && dest.Length - offset > 0)
            {
                ...
                if (formatAs == UriFormat.UriEscaped && NotAny(Flags.UserEscaped) && InFact(Flags.E_PathNotCanonical))
                {
                    ...
                    // As a workaround, re-create the Span in a way that avoids analysis
                    ReadOnlySpan<char> copySpan = MemoryMarshal.CreateReadOnlySpan(ref copy.GetPinnableReference(), copy.Length);
                    UriHelper.EscapeString(copySpan, ref dest, checkExistingEscaped: !IsImplicitFile, UriHelper.UnreservedReservedExceptQueryFragmentChars);
                    start = dest.Length;
        ...
        protected static string EscapeString(string? str) =>
            ...
            UriHelper.EscapeString(str, checkExistingEscaped: true, UriHelper.UnreservedReservedExceptQueryFragmentChars);
