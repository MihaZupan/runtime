...
namespace System
{
    internal static class UriHelper
    {
        ...
        //
        internal static unsafe bool TestForSubPath(char* selfPtr, int selfLength, char* otherPtr, int otherLength, bool ignoreCase)
        {
            int i = 0;
            char chSelf;
            char chOther;
                    ...
                    for (; next < end; ++next)
                    {
                        ...
                        else if ((unescapeMode & UnescapeMode.Escape) != 0)
                        {
                            ...
                            if (ch == rsvd1 || ch == rsvd2 || ch == rsvd3)
                            {
                                ...
                                && (ch <= '\x1F' || CharHelper.IsInInclusiveRange(ch, '\x7F', '\x9F')))
                            {
                                // found an unescaped reserved character -> escape it
                                escapeReserved = true;
                                break;
        ...
        {
            if (ch <= '\x1F' || CharHelper.IsInInclusiveRange(ch, '\x7F', '\x9F'))
            {
                return true;
            }
        ...
        internal static unsafe string StripBidiControlCharacter(char* strToClean, int start, int length)
        {
            ...
            for (int i = 0; i < length; ++i)
            {
                ...
                if (!CharHelper.IsInInclusiveRange(c, '\u200E', '\u202E') || !IsBidiControlCharacter(c))
                {
                    cleanStr[count++] = c;
                }
            }
