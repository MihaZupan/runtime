...
namespace System.PrivateUri.Tests
{
    public static class UriTests
    {
        ...
        [Fact]
        public static void Uri_Issue1487_DoesNotRegress()
        {
            // https://github.com/dotnet/runtime/issues/1487

            const char LeftToRightMark = '\u200e';

            //Debugger.Launch();

            // Assert.Throws<UriFormatException>(() => new Uri($"/\\{LeftToRightMark}‎//", UriKind.Absolute));
            // Assert.Throws<UriFormatException>(() => new Uri($"\\/{LeftToRightMark}", UriKind.Absolute));
            Assert.Throws<UriFormatException>(() => new Uri($"\\\\\\\\\\{LeftToRightMark}", UriKind.Absolute));
            // Assert.Throws<UriFormatException>(() => new Uri("/\\\\-Ā\r", UriKind.Absolute));
        }
    }
}
